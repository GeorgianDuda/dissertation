...
SELECT p.p_brand, SUM(l.l_quantity) AS total_qty
FROM lineitem l
...
SELECT p.p_type, SUM(l.l_extendedprice) AS total_price
FROM lineitem l
JOIN part p ON l.l_partkey = p.p_partkey
GROUP BY p.p_type;

-- Q005: Count of orders by order priority
SELECT o.o_orderpriority, COUNT(DISTINCT o.o_orderkey) AS num_orders
FROM orders o
GROUP BY o.o_orderpriority;

-- Q006: Average quantity per part container
SELECT p.p_container, AVG(l.l_quantity) AS avg_qty
FROM lineitem l
JOIN part p ON l.l_partkey = p.p_partkey
...
SELECT p.p_mfgr, SUM(ps.ps_supplycost * ps.ps_availqty) AS total_cost
FROM partsupp ps
...
SELECT l.l_shipmode, SUM(l.l_extendedprice) AS total_price
FROM lineitem l
GROUP BY l.l_shipmode;

-- Q012: Count of lineitems grouped by return flag
SELECT l.l_returnflag, COUNT(*) AS item_count
FROM lineitem l
GROUP BY l.l_returnflag;

-- Q013: Average quantity of items grouped by part size
SELECT p.p_size, AVG(l.l_quantity) AS avg_qty
FROM lineitem l
JOIN part p ON l.l_partkey = p.p_partkey
GROUP BY p.p_size;
...
SELECT p.p_brand, MAX(l.l_discount) AS max_discount
FROM lineitem l
JOIN part p ON l.l_partkey = p.p_partkey
GROUP BY p.p_brand;
...
SELECT s.s_name, MIN(ps.ps_supplycost) AS min_cost
FROM partsupp ps
JOIN supplier s ON ps.ps_suppkey = s.s_suppkey
GROUP BY s.s_name;

-- Q018: Total shipped quantity grouped by ship year
SELECT EXTRACT(YEAR FROM l.l_shipdate) AS ship_year, SUM(l.l_quantity) AS total_qty
FROM lineitem l
GROUP BY ship_year;

-- Q019: Total inventory value (supply cost * quantity) grouped by container
SELECT p.p_container, SUM(ps.ps_availqty * ps.ps_supplycost) AS total_value
FROM partsupp ps
JOIN part p ON ps.ps_partkey = p.p_partkey
GROUP BY p.p_container;

-- Q020: Average order total value grouped by region
SELECT r.r_name, AVG(o.o_totalprice) AS avg_order_total
FROM orders o
JOIN customer c ON o.o_custkey = c.c_custkey
JOIN nation n ON c.c_nationkey = n.n_nationkey
JOIN region r ON n.n_regionkey = r.r_regionkey
...
SELECT 
    ...
    o.o_totalprice
FROM orders o
WHERE o.o_clerk LIKE 'Clerk#000000%'
...
SELECT 
    ...
    l.l_quantity
FROM lineitem l
WHERE l.l_shipdate < DATE '1996-12-01'
  AND l.l_commitdate > l.l_shipdate
  AND l.l_quantity > 30;
...
SELECT 
    ...
    p.p_size
FROM part p
WHERE p.p_name NOT LIKE '%COPPER%'
...
WHERE o.o_orderpriority = '2-HIGH'
  AND o.o_orderdate BETWEEN DATE '1995-03-01' AND DATE '1995-03-31'
  AND l.l_quantity BETWEEN 10 AND 30;
...
SELECT 
    ...
    s.s_comment
FROM supplier s
WHERE s.s_comment LIKE '%unusual%'
  AND s.s_phone NOT LIKE '27-%';
...
SELECT 
    ...
    o.o_clerk
FROM orders o
WHERE o.o_totalprice BETWEEN 100000 AND 200000
  AND o.o_clerk LIKE '%123';
...
JOIN part p ON l.l_partkey = p.p_partkey
WHERE p.p_type = 'SMALL PLATED BRASS';
...
FROM orders o
WHERE EXISTS (
    SELECT 1 FROM lineitem l WHERE l.l_orderkey = o.o_orderkey AND l.l_discount > 0.08
);
...
WHERE EXISTS (
    ...
    JOIN part p ON ps.ps_partkey = p.p_partkey
    WHERE ps.ps_suppkey = s.s_suppkey AND p.p_size = 50
);
...
WHERE p.p_retailprice > (
    SELECT MAX(p2.p_retailprice) FROM part p2 WHERE p2.p_size = 15
);
...
FROM orders o
WHERE NOT EXISTS (
    SELECT 1 FROM lineitem l WHERE l.l_orderkey = o.o_orderkey AND l.l_quantity >= 30
);
...
FROM supplier s
WHERE NOT EXISTS (
    SELECT 1 FROM partsupp ps WHERE ps.ps_suppkey = s.s_suppkey AND ps.ps_supplycost > 500
);
...
WHERE EXISTS (
    ...
    JOIN supplier s ON ps.ps_suppkey = s.s_suppkey
    JOIN nation n ON s.s_nationkey = n.n_nationkey
    WHERE ps.ps_partkey = l.l_partkey AND n.n_name = 'JAPAN'
);
...
WHERE NOT EXISTS (
    ...
    JOIN supplier s ON ps.ps_suppkey = s.s_suppkey
    JOIN nation n ON s.s_nationkey = n.n_nationkey
    WHERE ps.ps_partkey = p.p_partkey AND n.n_name != 'UNITED STATES'
);
...
WHERE o.o_totalprice > 50000
  AND EXISTS (
    SELECT 1 FROM lineitem l WHERE l.l_orderkey = o.o_orderkey AND l.l_discount > 0.05
);
...
WHERE NOT EXISTS (
    ...
    WHERE l.l_orderkey = o.o_orderkey AND l.l_shipmode != 'AIR'
);
...
WHERE NOT EXISTS (
    SELECT 1 FROM partsupp ps
    JOIN part p ON ps.ps_partkey = p.p_partkey
    WHERE ps.ps_suppkey = s.s_suppkey AND p.p_brand = 'Brand#23'
);
...
WHERE NOT EXISTS (
    ...
    WHERE o.o_custkey = c.c_custkey AND o.o_orderdate BETWEEN DATE '1995-01-01' AND DATE '1995-12-31'
);
...
FROM part p
WHERE p.p_size > (
    SELECT AVG(p2.p_size) FROM part p2 WHERE p2.p_container = 'MED BOX'
);
...
WHERE NOT EXISTS (
    SELECT 1 FROM lineitem l
    JOIN part p ON l.l_partkey = p.p_partkey
    WHERE l.l_orderkey = o.o_orderkey AND p.p_size < 5
);
...
WHERE (
    ...
    FROM partsupp ps
    JOIN part p ON ps.ps_partkey = p.p_partkey
    WHERE ps.ps_suppkey = s.s_suppkey AND p.p_container = 'LG CASE'
) > 1;
...
WHERE NOT EXISTS (
    ...
    FROM lineitem l
    JOIN part p ON l.l_partkey = p.p_partkey
    WHERE l.l_orderkey = o.o_orderkey AND p.p_brand != 'Brand#45'
);
...
SELECT o.o_orderkey,
       (SELECT AVG(date_diff('day', l.l_shipdate, l.l_receiptdate))
        FROM lineitem l
        WHERE l.l_orderkey = o.o_orderkey) AS avg_shipping_delay
FROM orders o
...
SELECT o.o_orderkey, AVG(l.l_quantity) AS avg_qty
FROM orders o
JOIN lineitem l ON o.o_orderkey = l.l_orderkey
...
SELECT o.o_orderkey, o.o_custkey, SUM(l.l_quantity) AS total_qty
FROM orders o
JOIN lineitem l ON o.o_orderkey = l.l_orderkey
...
WITH order_value AS (
    SELECT o.o_orderkey, SUM(l.l_extendedprice * (1 - l.l_discount)) AS val
    FROM orders o
    JOIN lineitem l ON o.o_orderkey = l.l_orderkey
    GROUP BY o.o_orderkey
)
