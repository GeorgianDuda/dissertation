...
def main():
    ...
    for _, row in df.iterrows():
        ...
        normalized_metrics = analyze_query(row['Normalised'], query_id, 'Normalized')
        denormalized_metrics = analyze_query(row['Denormalised'], query_id, 'Denormalized')

        merged_row = {
            'QueryId': query_id,
            'Description': description
        }
        merged_row.update(normalized_metrics)
        merged_row.update(denormalized_metrics)
    ...
    print("Merged analysis saved to Query_Analysis.xlsx")

    if error_log:
        error_df = pd.DataFrame(error_log)
        error_df.to_excel("Query_Errors_Log.xlsx", index=False)
        print(f"{len(error_log)} queries had issues. See Query_Errors_Log.xlsx for details.")
